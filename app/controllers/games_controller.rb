...
def handle_skin_snippet_show
  return redirect_to games_path, alert: "No skin snippet data available" if @game_data.empty?

  @skin = Skin.where.not(splash_art: [ nil, "" ]).order("RANDOM()").first

  if @skin && @skin.splash_art.present?
    @skin_splash_url = @skin.splash_art

    # Adjusted zoom and position ranges for better visibility
    @zoom_level = rand(200..650)  # Increased minimum zoom

    # Adjusted position ranges to keep content more centered
    # These ranges ensure we don't move too far from center
    @position_x = 0    # Reduced range to prevent moving too far left
    @position_y = rand(-400..400)    # Reduced range to prevent moving too far up

    # Calculate the opposite position values to ensure we don't cut off too much
    @position_x = -(@position_x + rand(0..0)) if rand < 0.5
    @position_y = -(@position_y + rand(2..12)) if rand < 0.5

    @available_skins = Skin.where.not(id: @skin.id)
                          .where.not(splash_art_centered: [ nil, "" ])
                          .order("RANDOM()")
                          .limit(26)
                          .map { |skin| format_skin_data(skin) }

    @available_skins << format_skin_data(@skin)
    @available_skins.shuffle!

    session[:current_skin] = {
      id: @skin.id,
      name: @skin.name,
...
def format_skin_data(skin)
  OpenStruct.new(
    id: skin.id,
    name: skin.name,
    champion_name: skin.champion&.name,
    splash_art_url: skin.splash_art_centered  # Changed to match your column name
  )
end
# ABILITY SECTION
